...
import { DUBLIN_POINTS, DUBLIN_QUESTS } from "./dublin.ts";

/** Real townlands, real category mix, distances that actually fall inside
 *  their tier tolerance. No lorem and no invented placenames: fake content
 *  hides exactly the layout problems real content causes.
 *
 *  Two regions, because the product is Ireland wide and a corpus with one
 *  county in it hides every question about what happens when a quest is two
 *  hundred kilometres away. North Dublin is the home region, generated by
 *  `scripts/build-dublin.mjs`; Clare is the rest of the country. */
