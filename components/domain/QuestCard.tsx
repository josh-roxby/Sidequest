  ...
  return (
    <Link href={`/quests/${quest.id}`} className="block active:scale-[0.99]"
      ...
      <Card flag={flag} className="flex gap-3.5">
        <Plate ratio="1/1" plate={quest.thumb} label={quest.townland} sizes="72px" className="w-[72px] shrink-0" />
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-2 flex flex-wrap items-center gap-2">
            <ShapeChip shape={quest.shape} tip={false} />
            <Data className="text-[11px] uppercase text-mute">
              {formatDistance(quest.distanceM)} ·{" "}
              {formatDuration(estimateDurationS(quest.distanceM, {
                surface: quest.surface, ascentM: quest.ascentM,
                dwellS: total * 240,
              }))}
            </Data>
